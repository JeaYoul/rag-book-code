...
| `pipeline.py` | 전체 흐름. 서브쿼리마다 15 → 10 → 5. 컨텍스트는 조각 본문 + 출처 번호. `--ask` 면 LLM 에게 |
...
- "낙산균이 대장 염증에 미치는 효과와 기전은?" → 영어로 온전히 번역, 후보 30 → 서로 다른 논문 5편. 다섯 조각 전부 **영어 판**에서 걸렸다.
- 복합 질문 → LLM 이 서브쿼리 6개로 쪼갬, 23조각·20편, 컨텍스트 3만 3천 자, `[n]` 인용이 달린 답. 리랭커는 Qwen 주력이 응답(폴백 없음), 점수 0.98~1.00.
- 처음 돌렸을 때 두 가지가 조용히 틀렸다: `content_for_llm` 이 DB 에서 전부 비어 있어 본문이 빈 채로 갔고, Qwen 추론 모델이 답을 `reasoning_content` 에 넣어 번역·분해·답이 전부 빈 문자열이었다. 둘 다 이 저장소에 고쳐 두었다. 오류 없이 틀리는 것이 가장 무섭다 — 4장.
...
- 이 저장소에서 자동 검증한 것: 리라이팅 규칙, 두 벌 병합, 가짜 리랭커, 논문당 둘 규칙, 메모리 백엔드 전체 흐름. `PgBackend` 와 진짜 리랭커·LLM 은 위와 같이 Spark2 에서 확인했다.
