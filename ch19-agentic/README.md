...
- 이 저장소에서 자동 검증한 것: stdio·HTTP 두 통로로 MCP 서버 호출, 상태 표시등, 가짜 각본으로 도는 루프(예산 · 도구별 한도 · 인용 가드 · 무인용 문단 가드).

## Spark2 에서 돌려 본 결과 (2026-09-03)

- "설포라판과 부티르산의 HDAC 억제 시너지와 표적은?" → 5바퀴, 도구 11회. 모델이 스스로 영어 서브쿼리 넷을 만들어 `search_papers` 를 부르고, ChEMBL 로 두 화합물을 찾아 활성을 받고, `get_paper_details` 로 본문을 더 읽었다. 그 다음 답을 썼다. 아무도 순서를 정해 주지 않았다.
- 도구별 예산이 실제로 걸렸다. `get_paper_details` 를 세 번째 부르려 하자 코드가 "예산 초과"를 돌려주었고, 모델은 가진 것으로 답을 썼다.
- 처음 실행에서는 논문 서버가 아예 붙지 않았다. 서버가 시작하며 찍은 임베딩 모델 로딩 메시지가 표준출력으로 나가 규약을 더럽혔기 때문이다. **stdio 서버의 표준출력은 규약 전용이다.** 그래서 `PROTO_OUT` 으로 분리하고 `print` 는 전부 stderr 로 보냈다. 클라이언트도 JSON 이 아닌 줄은 건너뛴다.
- 그때 모델은 논문 없이 답을 썼는데, "근거를 찾지 못했다"고 해 놓고 그 아래에 "일반적 과학적 배경(참고용)" 이라는 절을 만들어 기억으로 채웠다. 규칙 1 을 반만 지킨 것이다. 그래서 규칙 5 를 넣고, 인용 없이 긴 문단을 찾아 표시하는 `flag_uncited_claims` 를 붙였다. 막지는 못한다. 사람 눈에 띄게 할 뿐이다.
- `ncbi_search` 는 단어를 넷 이상 AND 로 묶으면 PubMed 가 빈 결과를 준다. 도구가 고장난 것이 아니라 정말 없는 것이다. 이것을 구분하지 못하면 사람은 도구를 의심하고, 모델은 기억으로 채운다.
